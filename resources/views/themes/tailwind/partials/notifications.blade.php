    ...
    <div id="notification-list" @click.away="open = false" class="relative flex items-center h-full" x-data="{ open: false }">
        <div id="notification-icon relative">
            <button @click="open = !open" class="relative p-1 ml-3 text-gray-400 transition duration-150 ease-in-out rounded-full hover:text-gray-500 focus:outline-none focus:text-gray-500 focus:bg-gray-100">
                <svg class="text w-6 h-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
...
@endif
    ...
    @endif
        <div class="notification-menu bg-white rounded-md border border-gray-100 @if(!isset($show_all_notifications)){{ 'shadow-md' }}@endif" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">

            @if(!isset($show_all_notifications))
            <div id="notification-header">
                <div id="notification-head-content" class="dark-section text flex items-center w-full px-3 py-3 text-gray-600 border-b border-gray-200">
            ...
            <div id="notifications-none" class="dark-section text @if($notifications_count > 0){{ 'hidden' }}@endif @if(isset($show_all_notifications)){{ 'bg-gray-150' }}@endif flex items-center justify-center h-24 w-full text-gray-600 font-medium">
            ...
            @if(!isset($show_all_notifications))
                <div id="notification-footer" class="dark-section text flex items-center justify-center py-3 text-xs font-medium text-gray-600 bg-gray-100 border-t border-gray-200 ">
